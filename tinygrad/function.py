...
class Reciprocal(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.const(1).e(BinaryOps.IDIV, x)
    return self.ret
...
class Log(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer: return grad_output.e(BinaryOps.IDIV, self.x)
...
class Sqrt(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return grad_output.e(BinaryOps.IDIV, self.ret.e(BinaryOps.MUL, self.ret.const(2)))
...
class Sigmoid(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.const(1).e(BinaryOps.IDIV, x.const(1).e(BinaryOps.ADD, x.e(BinaryOps.MUL, x.const(-1/math.log(2))).e(UnaryOps.EXP2)))
    return self.ret
...
class Max(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    ...
    return max_is_1s.e(BinaryOps.IDIV, div).cast(grad_output.dtype).e(BinaryOps.MUL, grad_output.expand(self.x.shape))
