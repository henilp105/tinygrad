...
class UnaryOps(Enum):
  """A -> A (elementwise)"""
  EXP2 = auto(); LOG2 = auto(); CAST = auto(); SIN = auto(); SQRT = auto(); NEG = auto(); BITCAST = auto() # noqa: E702
...
InterpretedFlopCounter: Dict[Op, Callable] = {
  ...
  **{op:lambda self: FlopCounter(self.shape, self.consume_flops() + prod(self.shape), self.mem) for op in UnaryOps if op is not UnaryOps.CAST and op is not UnaryOps.BITCAST},
